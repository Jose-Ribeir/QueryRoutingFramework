...
function extractConclusionsContent(): (string | { type: 'image'; src: string; alt: string; caption?: string })[] {
  ...
  // Get conclusions pages (128-143 based on section detection)
  const conclusionsPages = extractedContent.pages.filter((page: PageContent) =>
    page.pageNumber >= 128 && page.pageNumber <= 143
  );

  // Get conclusions images (if any)
  const conclusionsImages = imageMapping.allImages.filter((img: ImageInfo) =>
    img.pageNumber >= 128 && img.pageNumber <= 143
  );

  console.log(`Found ${conclusionsPages.length} conclusions pages and ${conclusionsImages.length} images`);

  const result: (string | { type: 'image'; src: string; alt: string; caption?: string })[] = [];

  // Sort images by page number
  const sortedImages = conclusionsImages.sort((a: ImageInfo, b: ImageInfo) => a.pageNumber - b.pageNumber);

  let imageIndex = 0;

  for (const page of conclusionsPages) {
...
function main() {
  console.log('Extracting conclusions content...');
  const conclusionsContent = extractConclusionsContent();

  console.log(`Extracted ${conclusionsContent.length} content items (text + images)`);

  // Save to a temporary file for inspection
  const outputPath = path.join(process.cwd(), 'scripts', 'conclusions-content.json');
  fs.writeFileSync(outputPath, JSON.stringify(conclusionsContent, null, 2));

  console.log(`Conclusions content saved to: ${outputPath}`);

  // Count text vs images
  const textCount = conclusionsContent.filter(item => typeof item === 'string').length;
  const imageCount = conclusionsContent.filter(item => typeof item === 'object').length;
