...
function main() {
  ...
  for (const section of sections) {
    ...
    // Separate text and images, removing duplicates
    ...
    items.forEach((item) => {
      if (typeof item === 'string') {
        ...
        if (!imageMap.has(item.src)) {
          ...
          console.log(`  ⚠️  Removing duplicate: ${path.basename(item.src)}`);
        }
      }
    });
    ...
    console.log(`  Found ${textItems.length} text items and ${imageItems.length} unique images`);
    ...
    // Strategy: Place images in figure number order
    // Try to place each image after its target text, but maintain figure number order
    // If target comes too early, place image as early as possible while maintaining order
    
    const newItems: ContentItem[] = [];
    let textIndex = 0;
    let imageIndex = 0;
    let lastPlacedTextIndex = -1;
    ...
    for (const img of imageItems) {
      ...
      if (img.targetTextIndex >= 0 && img.targetTextIndex < textItems.length) {
        // Has target - try to place it there, but ensure figure number order
        // Must be placed after the last image's position
        insertAfter = Math.max(
          lastPlacedTextIndex + 1,
          img.targetTextIndex
        );
      } else {
        // No target - place after last image or distribute
        insertAfter = lastPlacedTextIndex + 1;
      }
      ...
      lastPlacedTextIndex = insertAfter;
    }
    
    // Now build the content array, ensuring no consecutive images
    textIndex = 0;
    imageIndex = 0;
    ...
    // Add any remaining images at the end (but maintain order)
    while (imageIndex < imagePlacements.length) {
      const placement = imagePlacements[imageIndex];
      const lastWasImage = newItems.length > 0 && 
        typeof newItems[newItems.length - 1] === 'object' && 
  ...
  console.log('Done! Images reordered by figure number and duplicates removed.');
}
